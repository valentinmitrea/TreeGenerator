...
namespace lab {
	...
	private:
		...
		void updateGeometry(){
			...
			vertices.push_back(CamVertexFormat(position + glm::normalize(forward)*axis_length, glm::vec3(0, 0, 1)));
			vertices.push_back(CamVertexFormat(position, glm::vec3(1, 0, 0)));
			vertices.push_back(CamVertexFormat(position + glm::normalize(right)*axis_length, glm::vec3(1, 0, 0)));
			vertices.push_back(CamVertexFormat(position, glm::vec3(0, 1, 0)));
			vertices.push_back(CamVertexFormat(position + glm::normalize(up)*axis_length, glm::vec3(0, 1, 0)));
			glBindBuffer(GL_ARRAY_BUFFER, vbo);
